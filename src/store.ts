...
export class MemoryStore {
   ...
   */
  async remember(request: RememberRequest): Promise<MemoryObject> {
  ...
  recall(request: RecallRequest = {}): MemoryObject[] {
    ...
    if (request.tags && request.tags.length > 0) {
      results = results.filter(m => 
        request.tags!.some(tag => m.tags.includes(tag))
      );
    }
   ...
   */
  async forget(request: ForgetRequest): Promise<MemoryObject | null> {
   ...
   * Update a memory
   */
  async update(id: string, updates: Partial<RememberRequest>): Promise<MemoryObject | null> {
    const memory = this.memories.get(id);
    if (!memory || memory.deleted) return null;
  ...
  async export(): Promise<ExportResponse> {
    ...
    // Sign manifest
    manifest.signature = await sign(
      JSON.stringify({ ...manifest, signature: undefined }),
  ...
  private loadOrCreateIdentity(): MemoryIdentity {
  ...
  private loadMemories(): void {
    ...
    for (const line of lines) {
      try {
        const memory: MemoryObject = JSON.parse(line);
  ...
  private async signMemory(memory: Omit<MemoryObject, 'signature'>): Promise<MemorySignature> {
